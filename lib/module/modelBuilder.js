 ...
 */
function ModelBuilder(vm, mb, forceValidate = false, buildModel = Object.assign({}, mb)) {
...
function valueValidate(mb, vm, buildModel, k, forceValidate) {
    switch (typeof mb[k].validate) {
        case common_1.ModelTypes.FUNCTION:
            if (mb[k].validate(vm[mb[k].key])) {
                ...
                checkDefaults(mb, buildModel, k, forceValidate);
            break;
...
function checkDefaults(mb, buildModel, k, forceValidate) {
    if (mb[k].hasOwnProperty("default"))
        ...
        throw new Error(`custom function failed to validate value`);
}
